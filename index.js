...
function isSelfChat(chatId, botNumber) {
    ...
    return normalizedChatId === normalizedBotNumber;
}
...
async function startXeonBotInc() {
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            // Only process status messages and self-chat messages
            ...
            // Only allow self-chat messages (user messaging themselves)
            if (!isSelfChat(chatId, botNumber)) {
    ...
    XeonBotInc.ev.on('connection.update', async (s) => {
        ...
        if (connection == "open") {
            ...
            await XeonBotInc.sendMessage(botNumber, { 
                ...
                \n✅ Security: Enhanced - Only responds to self-chat`,
                contextInfo: {
                    forwardingScore: 1,
                    isForwarded: true,
                    forwardedNewsletterMessageInfo: {
    ...
    XeonBotInc.ev.on('group-participants.update', async (update) => {
        // Silently ignore all group participant updates
        return;
    });

    // Only process status messages from self
    XeonBotInc.ev.on('messages.upsert', async (m) => {
        if (m.messages[0].key && m.messages[0].key.remoteJid === 'status@broadcast') {
            const botNumber = XeonBotInc.user.id.split(':')[0] + '@s.whatsapp.net';
            const senderId = m.messages[0].key.participant || m.messages[0].key.remoteJid;
            
            // Only process status from self
    ...
    // Disable status update handling
    XeonBotInc.ev.on('status.update', async (status) => {
        // Silently ignore all status updates
        return;
    });

    // Disable reaction handling
    XeonBotInc.ev.on('messages.reaction', async (status) => {
        // Silently ignore all reactions
        return;
    });
    // ===== END SECURITY MODIFICATION =====
